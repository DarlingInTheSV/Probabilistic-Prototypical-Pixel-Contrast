...
def vis(img, title, crop_box=None, draw_label=False, overlay=False):
    rows, cols = 1, 1
    ...
    if draw_label:
        ...
        subplotimg(axs, img[0][0], f'{title}')
        subplotimg(axs, img[1], f'{title}', alpha=0.50, cmap='jet')
    else:
        subplotimg(axs, img, f'{title}')
    if crop_box:
        from matplotlib.patches import Rectangle
        crop_box = crop_box[0]
        rect = Rectangle(crop_box[0], crop_box[1], crop_box[2], linewidth=1, edgecolor='r', facecolor='none')
        axs.add_patch(rect)
    ...
    # for ax in axs.flat:
    axs.axis('off')
    plt.savefig(f"/home/lsy/Desktop/norm/{title}.png", dpi=300)
...
@UDA.register_module()
class PPPCDark(UDADecorator):

    def __init__(self, **cfg):
        super(PPPCDark, self).__init__(**cfg)
        # basic setup
        self.local_iter = 0
        # self.sche_sch = RampdownScheduler(0, 40000, 0, 1.0, 0, -5.0, 1)
        self.sche_sch = RampdownScheduler(0, 40000, 0, 1.0, 0, -10.0, 1)
    ...
    # dark exclusive
    def random_crop(self, images, gt_seg, true_gt=None, prod=True, proto_sigma=None, dataset=None):
        if prod:
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img, target_img_metas, target_gt_semantic_seg):
        ...
        if self.pseudo_random_crop:
            ...
            (target_img, weak_target_img), pseudo_label = \
                self.random_crop((target_img, weak_target_img), pseudo_label,
                                                                                    prod=self.prod,
        ...
        #     self.threshold_strong += 0.03
        #     self.threshold_strong = min(self.threshold_strong, 0.9)
        # log_vars["sche_value"] = self.threshold_strong
        ...
        source_losses = self.get_model().forward_train(img, img_metas, gt_semantic_seg, return_feat=False,
                                                       ...
                                                       scale=sche.value, log_vars=log_vars)
